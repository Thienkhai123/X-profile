...
import ApplicationHistoryEdit from 'common/presentation/Pages/ApplicationHistoryEdit'
...
const ApplicationHistoryPage = () => {
  ...
  const loading = useSelector(
    (state) =>
      ...
      selectLoading(state, APP_TYPES.CAMPAIGN.USERACCEPTINTERVIEWCAMPAIGNS) ||
      selectLoading(
        state,
        APP_TYPES.CAMPAIGN.GETUSERACCEPTINTERVIEWCAMPAIGNS
      ) ||
      selectLoading(state, APP_TYPES.CAMPAIGN.REJECT) ||
      selectLoading(state, APP_TYPES.CAMPAIGN.REVIEW)
  )

  useEffect(() => {
    dispatch(getAllAppliedCampaigns({ status: 100 }))
  ...
  return (
    <>
      ...
      <div className="flex-1 pb-[84px] xl:pt-[44px] bg-white xl:bg-inherit">
        <Head>
          <title>Lịch sử ứng tuyển - X-Profile</title>
        </Head>
        ...
        <div className="flex flex-col xl:flex-row relative xl:w-[1140px] w-full gap-[20px] mx-auto xl:mt-6">
          ...
          <div className="px-4 xl:px-0 xl:w-2/3  w-full">
            <div className=" xl:bg-white xl:p-8 rounded-[12px]">
              <ApplicationHistoryEdit
                {...userProfile}
