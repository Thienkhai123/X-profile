...
import ProductOwnerSvg from 'common/presentation/Pages/landing/productOwner/productOwnerSvg'
import ProductOwnerSvgMobile from 'common/presentation/Pages/landing/productOwner/productOwnerSvgMobile'
...
const ProductLandingPage = () => {
  ...
  const download = (name = 'ProductOwner') => {
    fetch('/images/Landing/ITCareerPO.png')
      .then((response) => response.blob())
      .then((blob) => {
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `ProductOwner.png`
  ...
  const handleShareBlog = () => {
    if (window !== undefined) {
      window.open('https://blog.xprofile.vn/tag/product-owner/')
    }
  }
  ...
  const handleShareInsMobile = async () => {
    const response = await fetch('/images/Landing/ITCareerPOMobile.png')
    const blob = await response.blob()
    const filesArray = [
      new File([blob], 'product_owner.jpg', {
        type: 'image/jpeg',
  ...
  const shareFacebook = () => {
    const buttonShareFB = document.getElementById('shareFacebookPO')
    if (buttonShareFB) {
      buttonShareFB.click()
    }
  }
  ...
  const shareLinkedin = () => {
    const buttonShareLinkedIn = document.getElementById('shareLinkedInPO')
  ...
  useEffect(() => {
    const element = document.getElementById('download-survey-productOwner')
    if (element) {
  ...
  return (
    <div className="bg-yellow-survey">
      <div className="flex-1 max-w-[1240px] mx-auto px-[24px] xl:px-0 ">
        <MetaSeo
          title="Product Owner - Bạn là ai trong thế giới IT - X-Profile"
          titleContent={'Product Owner'}
          descContent={'Product Owner'}
          urlContent="https://xprofile.vn/itcareers/productOwner"
          imageContent="https://he44r2a3tgobj.vcdn.cloud/p/Website/SurveyITCareers/X-Profile_PO-1.png"
        />
            ...
            <div className="xl:pt-[80px] pt-0">
              <div className="flex flex-col xl:gap-[12px] gap-[8px] max-w-[626px] xl:mb-[80px] mb-[48px]">
                <p className="sm:text-p48 text-h3 text-neutral xl:max-w-[100%] ">
                  Hura! Bạn chính là một
                </p>
                <p className="sm:text-p48 text-h3 text-neutral xl:max-w-[100%]">
                  NHẠC TRƯỞNG ĐẦY CHIẾN THUẬT
                </p>
                <p className="sm:text-p20 text-p14 italic font-[300] text-semantic-green">
                  {`/ Muốn sang thì bắc cầu Kiều, muốn flow hiệu quả thì nhờ PO! /`}
                </p>
              </div>
              <div className="xl:hidden block justify-end top-0 mb-[32px]">
                <Image
                  src="/images/Landing/ProductOwner.png"
                  width={600}
                  height={600}
                  objectFit="contain"
                  quality={100}
          ...
          <div className="flex justify-center">
            <div className="w-[54.313vw]  h-[104.271vw] relative">
              <Image
                ...
                priority
                objectFit="contain"
                src="/images/Landing/ITCareerPOMobile.png"
                quality={100}
              />
            </div>
          </div>
