...
import DeveloperSvg from 'common/presentation/Pages/landing/developer/developerSvg'
import DeveloperSvgMobile from 'common/presentation/Pages/landing/developer/developerSvgMobile'
...
const DeveloperLandingPage = () => {
  ...
  const download = () => {
    fetch('/images/Landing/devDow.png')
      .then((response) => response.blob())
      .then((blob) => {
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `Developer.png`
  ...
  const handleShareBlog = () => {
    if (window !== undefined) {
      window.open('https://blog.xprofile.vn/tag/developer/', '_blank')
    }
  }
  ...
  const shareFacebook = () => {
    document.getElementById('shareFacebookDev').click()
  }

  const shareLinkedin = () => {
    document.getElementById('shareLinkedinDev').click()
  }

  const handleShareInsMobile = async () => {
    const response = await fetch('/images/Landing/devDowMobile.png')
    const blob = await response.blob()
    const filesArray = [
      new File([blob], 'developer.jpg', {
        type: 'image/jpeg',
  ...
  return (
    <div className="bg-yellow-survey">
      <div className="flex-1 max-w-[1240px] mx-auto px-[24px] xl:px-0 ">
        <MetaSeo
          title="Developer - Bạn là ai trong thế giới IT - X-Profile"
          titleContent={'Developer'}
          descContent={'Developer'}
          imageContent="https://he44r2a3tgobj.vcdn.cloud/p/Website/SurveyITCareers/X-Profile_Developer-1.png"
          urlContent="https://xprofile.vn/itcareers/developer"
        />
        ...
        <div className="py-[16px] xl:py-[80px]  xl:px-0">
          ...
          <div className="flex gap-[68px]">
            <div className="xl:pt-[80px] pt-0">
              <div className="flex flex-col xl:gap-[12px] gap-[8px] xl:max-w-[626px] w-full xl:mb-[80px] mb-[48px]">
                ...
                <p className="sm:text-p24 leading-[48px] text-p16 text-neutral font-light">
                  Người trong ngành gọi là Lập trình viên (Developer) nha!
                </p>
                <p className="sm:text-p20 text-p14 text-semantic-green italic">
                  / Sợ code chạy, sợ code không chạy.../
                </p>
              </div>
              <div className="xl:hidden block justify-end top-0 mb-[32px]">
                <Image
                  src="/images/Landing/dev.png"
                  width={600}
                  height={600}
                  alt=""
                  priority
              ...
              <div>
                <div className="hidden md:block xl:w-[622px] w-full relative">
                  <DeveloperSvg download={download} />
          ...
          <div className="flex justify-center">
            <div className="w-[48.313vw] h-[90.271vw] relative">
              <Image
                layout="fill"
                alt=""
                src="/images/Landing/devDowMobile.png"
                quality={100}
              />
            </div>
          </div>
