...
import TechLeadSvg from 'common/presentation/Pages/landing/techLead/techLeadSvg'
import TechLeadSvgMobile from 'common/presentation/Pages/landing/techLead/techLeadSvgMobile'
...
const TeachLeadLandingPage = () => {
  ...
  const download = (name = 'TechLead') => {
    fetch('/images/Landing/ITCareerTechLead.png')
      .then((response) => response.blob())
      .then((blob) => {
        const url = window.URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `TechLead.png`
  ...
  const handleShareBlog = () => {
    if (window !== undefined) {
      window.open('https://blog.xprofile.vn/')
    }
  }
  ...
  const handleShareInsMobile = async () => {
    const response = await fetch('/images/Landing/ITCareerTechLeadMobile.png')
    const blob = await response.blob()
    const filesArray = [
      new File([blob], 'teach_lead.jpg', {
        type: 'image/jpeg',
  ...
  const shareFacebook = () => {
    const buttonShareFB = document.getElementById('shareFacebookTechLead')
    if (buttonShareFB) {
      buttonShareFB.click()
    }
  }
  ...
  const shareLinkedin = () => {
    const buttonShareLinkedIn = document.getElementById('shareLinkedInTechLead')
  ...
  useEffect(() => {
    const element = document.getElementById('download-survey-techLead')
    if (element) {
  ...
  return (
    <div className="bg-yellow-survey">
      <div className="flex-1 max-w-[1240px] mx-auto px-[24px] xl:px-0 ">
        <MetaSeo
          title="Tech Lead"
          titleContent={'Tech Lead'}
          descContent={'Tech Lead'}
          urlContent="https://xprofile.vn/itcareers/techLead"
          imageContent="https://he44r2a3tgobj.vcdn.cloud/p/Website/SurveyITCareers/X-Profile_Techlead-1.png"
        />
        ...
        <div className="py-[16px] xl:py-[80px]  xl:px-0">
          <div className="flex justify-between  xl:mb-[40px] mb-[16px] pr-[28px] xl:pr-0 group">
            ...
            <div
              className="hidden xl:flex gap-[16px] items-center relative z-[100] cursor-pointer group"
              onClick={() => handleCreateTest()}
            ...
            <div className="xl:pt-[80px] pt-0">
              <div className="flex flex-col xl:gap-[12px] gap-[8px] max-w-[626px] xl:mb-[80px] mb-[48px]">
                ...
                <p className="sm:text-p24 text-p16 text-neutral">
                  Các huynh đệ ngành IT gọi bạn là “Tech Lead” hoặc “Technical
                  Leader”
                </p>
                <p className="sm:text-p20 text-p14 font-[300] italic text-semantic-green">
                  {`/ Vì sao em phải khóc? Có bấc thì để anh! /`}
                </p>
              </div>
              <div className="xl:hidden block justify-end top-0 mb-[32px]">
                <Image
                  src="/images/Landing/TeachLead.png"
                  priority
                  quality={100}
                  objectFit="contain"
                  width={600}
          ...
          <div className="flex justify-center">
            <div className="w-[54.313vw]  h-[104.271vw] relative">
              <Image
                ...
                src="/images/Landing/ITCareerTechLeadMobile.png"
                quality={100}
              />
            </div>
          </div>
...
export default TeachLeadLandingPage
