...
import { updateinviteCampaignSelected } from 'store/app/cvSlice'

export const ProfilePublicInviteCard = (props) => {
  const {
    isAuthentication,
    job,
    handleAction,
  ...
  const handleClickSave = async (recruitmentCampaignId) => {
    if (recruitmentCampaignId) {
      dispatch(updateinviteCampaignSelected(recruitmentCampaignId))
    }
  }
        ...
        >
          <div className="relative w-full h-[88px]  bg-background-worktop bg-[length:100%_100%]">
            <div
              ...
              style={{
                backgroundImage:
                  'linear-gradient(rgba(255, 255, 255, 0.7), rgba(255, 255, 255, 0.08))'
              }}
            ></div>
...
ProfilePublicInviteCard.propTypes = {
  job: PropTypes.shape({
...
ProfilePublicInviteCard.defaultProps = {
  job: {
    company: {},
    name: '',
    positions: 0,
