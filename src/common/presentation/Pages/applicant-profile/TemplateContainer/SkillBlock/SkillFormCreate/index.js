...
const SkillFormCreate = (props) => {
  ...
  const selectNewSkillAdvance = () => {
    if (isNaN(parseInt(keySearch.advanced))) {
      ...
      <div className="mt-6">
        ...
        <div className="relative" ref={refAdvancedOpt}>
          ...
          >
            ...
            <input
              placeholder=""
              className="outline-0 sm:text-p18 text-p12 text-neutral min-w-[200px] px-1 max-w-full placeholder:text-grey-3"
              maxLength={50}
              value={keySearch.advanced}
          ...
          {showOpt.advanced && (
            <div
              className={`bg-white rounded-lg max-h-[180px] w-full overflow-x-hidden absolute border border-grey-3 p-2 custom-scrollbar z-10`}
              style={{
              ...
              {listSkill?.advanced?.map((skill, ind) => {
                if (selectedSkill.advancedIds.includes(skill?.skillId)) {
                  return (
                    <div
                      key={ind}
                      className="flex justify-between items-center p-4 hover:bg-light-nude rounded-lg cursor-default"
                    >
...
export default SkillFormCreate
