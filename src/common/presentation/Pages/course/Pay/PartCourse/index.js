import React, { useState } from 'react'
import PropTypes, { element } from 'prop-types'
import { SwiperSlide } from 'swiper/react'
import { secondsToHms, secondsToHmsCourse } from 'store/helper/functionHelper'
import IntroduceCourseChapterTable from 'common/presentation/Card/IntroduceCourseChapterTable'
import { Slider } from 'common/presentation/Swiper'
import XProfileIcon from 'common/presentation/Icons'
import IntroduceCourseLessonChapterTable from 'common/presentation/Card/IntroduceCourseLessonChapterTable'

const PartCourse = (props) => {
  const { resultChapterTables = [], totalVideoCount, length } = props

  const sortPositon = (positionFirst, positionSeccond) => {
    return positionFirst?.position - positionSeccond?.position
  }

  const [reachEnd, setReachEnd] = useState(false)
  const [slider, setSlider] = useState({
    isBeginning: true,
    nextTran: true,
    prevTran: false,
    isEnd: false
  })

  return (
    <div className="relative w-full lg:w-[944px]">
      <div className="flex flex-col gap-[32px]">
        {resultChapterTables?.sort(sortPositon)?.map((element, ind) => {
          const {
            partCourseList,
            lessonCourseList,
            name,
            totalTimeParendId,
            totalTimeParendIdNull,
            totalVideo
          } = element
          return (
            <div key={ind}>
              <div className="flex justify-between mb-[24px] items-center">
                <p className="sm:text-p18-bold leading-[30px] text-p16-bold text-black ">
                  {name || 'Chương'}
                </p>
                <div className="flex items-center justify-between overflow-hidden gap-[8px]">
                  <div className="flex items-center gap-2 ">
                    <XProfileIcon name="play" />
                    <div className="flex items-center gap-1 ">
                      <p className="text-p16 leading-[28px] text-grey-1 line-clamp-1">
                        {totalVideo || 0} Videos
                      </p>
                      <span className="text-p16 leading-[28px] text-grey-1">
                        -
                      </span>
                      <p className="text-p16 leading-[28px] text-grey-1 line-clamp-1">
                        {secondsToHmsCourse(
                          totalTimeParendId + totalTimeParendIdNull || 0
                        )}
                      </p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-[20px]">
                {partCourseList?.length > 0 && (
                  <div className="relative">
                    <Slider
                      breakpoints={{
                        330: {
                          slidesPerView: 1,
                          slidesPerGroup: 1
                        },
                        1024: {
                          slidesPerView: 2.48,
                          slidesPerGroup: 1,
                          spaceBetween: 20
                        }
                      }}
                      rewind={false}
                      hasArrow={partCourseList?.length >= 3}
                      handleSlideChange={(e) => {
                        setSlider({
                          ...slider,
                          isBeginning: e?.isBeginning,
                          isEnd: e?.isEnd
                        })
                      }}
                      hasShadow={false}
                      stylePrev="p-[15.2px] border border-nude bg-white"
                      classNameLeft="absolute z-10 -left-[28px] top-1/2 -translate-y-1/2"
                      classNameRight="absolute z-10 -right-[28px] top-1/2 -translate-y-1/2"
                    >
                      {partCourseList?.sort(sortPositon)?.map((item, index) => {
                        return (
                          <SwiperSlide key={index}>
                            <IntroduceCourseChapterTable
                              {...item}
                              coursePartNumber={item?.name}
                              coursePartName={item?.chapterName}
                              sortPositon={sortPositon}
                            />
                          </SwiperSlide>
                        )
                      })}
                    </Slider>
                    {slider.isBeginning && partCourseList.length > 2 && (
                      <div className="bg-background-right-courseCombo absolute top-0 right-0 w-[182px] z-[5] h-full"></div>
                    )}
                    {slider.isEnd && partCourseList.length > 2 && (
                      <div className="bg-background-left-courseCombo absolute top-0 left-0 w-[182px] z-[5] h-full"></div>
                    )}
                    {!slider.isBeginning &&
                      !slider.isEnd &&
                      partCourseList.length > 2 && (
                        <div className="bg-background-right-courseCombo absolute top-0 right-0 w-[182px] z-[5] h-full"></div>
                      )}
                  </div>
                )}

                {lessonCourseList?.length > 0 && (
                  <Slider
                    breakpoints={{
                      330: {
                        slidesPerView: 1,
                        slidesPerGroup: 1
                      },
                      1024: {
                        slidesPerView: 2,
                        slidesPerGroup: 1,
                        spaceBetween: 0
                      }
                    }}
                    classNameSwiper="rounded-[16px] border border-grey-4 bg-white"
                    hasArrow={lessonCourseList.length > 2}
                    hasShadow={false}
                    stylePrev="p-[15.2px] border border-nude bg-white"
                    classNameLeft="absolute z-10 -left-[28px] top-1/2 -translate-y-1/2"
                    classNameRight="absolute z-10 -right-[28px] top-1/2 -translate-y-1/2"
                  >
                    {lessonCourseList?.sort(sortPositon)?.map((item, index) => {
                      return (
                        <SwiperSlide key={index} style={{ marginRight: '0px' }}>
                          <IntroduceCourseLessonChapterTable
                            padding="32px 0px 32px 80px "
                            paddingRightChil="92px"
                            gap="41px"
                            maxWidth="392px"
                            lessonList={item}
                            lessonLength={lessonCourseList.length}
                            sortPositon={sortPositon}
                          />
                        </SwiperSlide>
                      )
                    })}
                  </Slider>
                )}
              </div>
            </div>
          )
        })}
      </div>
    </div>
  )
}

PartCourse.propTypes = {
  resultChapterTables: PropTypes.array
}
PartCourse.defaultProps = {
  resultChapterTables: [
    {
      coursePartNumber: 'Phần 1',
      coursePartName: 'Giới thiệu tổng quan về khoá học 1'
    },
    {
      coursePartNumber: 'Phần 2',
      coursePartName: 'Giới thiệu tổng quan về khoá học 2'
    }
  ]
}
export default PartCourse
