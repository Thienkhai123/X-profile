...
const PropressBarJob = (props) => {
  ...
  const titleSoftSkillRef = useRef()
  const popUp = useRef()
  ...
  const profile = useSelector(selectProfilePositionSoftSkill)
  ...
  const submit = async (data) => {
    if (data.id !== 0) {
      ...
      dispatch(updateCompanySkills([...tempList]))
    } else {
      const skillItem = {
  ...
  const onClickDelete = async (id, ind) => {
    if (id !== 0) {
      ...
      dispatch(updateCompanySkills([...removeSkill]))
      toggleModalDelete()
    } else {
  ...
  const titleSoftSkillHeight = titleSoftSkillRef.current?.offsetHeight
  const popUpHeight = popUp.current?.offsetHeight

  const titleSoftSkillWidth = titleSoftSkillRef.current?.offsetWidth
  const popUpWidth = popUp.current?.offsetWidth / 2

  return (
    <div className="">
      <div className="flex justify-between items-center relative">
        <div className="mb-[4px] w-full grid grid-cols-2 gap-4 mb:gap-0">
          <div ref={titleSoftSkillRef} className="">
