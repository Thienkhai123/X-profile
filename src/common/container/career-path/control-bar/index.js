...
const DefaultControlBar = ({ toggleMap = () => {}, name = 'it' }) => {
  return (
    <div className="bg-white rounded-3xl p-2">
      <div className="flex gap-[10px]">
        <div
          id="control-bar-it-crp"
          className="rounded-2xl bg-light-blue py-5 px-6 lg:flex hidden  items-center cursor-pointer lg:w-[283px] justify-between"
          onClick={toggleMap}
        >
          <div className="flex items-cneter gap-2">
            <p className="text-p14-bold leading-[26px]">Bản đồ</p>
            <p className="text-p14 leading-[26px]">{MAPNAMES[name]}</p>
          </div>
          <div>
        ...
        <div
          id="control-bar-it-crp-mobile"
          className="rounded-2xl bg-light-blue py-5 px-6 lg:hidden flex gap-2 items-center cursor-pointer"
          onClick={toggleMap}
        >
          <XProfileIcon name="map2" />
        </div>
...
export const CareerPathControlBar = (props) => {
  const { map } = props
  const { name } = map || {}
  ...
  return (
    ...
    >
      <div className={`${!showMaps ? 'block' : 'hidden'}`}>
        <DefaultControlBar toggleMap={toggleMap} name={name} />
      </div>
      <div className={`${showMaps ? 'block' : 'hidden'}`}>
        <DetailControlBar toggleMap={toggleMap} {...props} name={name} />
      </div>
    </div>
  )
}
